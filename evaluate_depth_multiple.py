...
data_path = "../datasets/jpg_kitti_data/"
...
def evaluate(opt, weights_folder):
    ...
    if opt.ext_disp_to_eval is None:
        ...
        assert os.path.isdir(weights_folder), \
            "Cannot find a folder at {}".format(weights_folder)
    ...
    err_keys = ["abs_rel", "sq_rel", "rmse", "rmse_log", "a1", "a2", "a3"]
    err_vals = mean_errors.tolist()
    error = dict(zip(err_keys, err_vals)) 
...
if __name__ == "__main__":
    ...
    opt = options.parse()
    parent_path = os.path.expanduser(opt.load_weights_folder)
    all_weights = next(os.walk(opt.load_weights_folder))[1]
    all_errors = []
    for weight_folder in sorted(all_weights):
        error = evaluate(opt, os.path.join(parent_path, weight_folder))
