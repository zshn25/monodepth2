...
def calculate_means(pred, gt, n_objects, max_n_objects, usegpu):
    ...
    for i in range(bs):
        ...
        _mean_sample = _pred_masked_sample.sum(
            0) / (1e-7 + _gt_expanded_sample.sum(0))  # n_objects, n_filters
            
        if torch.sum(torch.isnan(_mean_sample)):
            print('mean causing nan', _pred_masked_sample.sum(
            0), _gt_expanded_sample.sum(
            0))
...
def calculate_variance_term(pred, gt, means, n_objects, delta_v, norm=2):
    ...
    if torch.sum(torch.isnan(var_term)):
        print('var', var_term)
    return var_term
...
def calculate_distance_term(means, n_objects, delta_d, norm=2, usegpu=True):
    ...
    if torch.sum(torch.isnan(dist_term)):
        print(dist_term, 'dist')
    return dist_term
...
class DiscriminativeLoss(nn.Module):
    ...
    def forward(self, input, target, n_objects, max_n_objects):
        ...
        one_hot = torch.cuda.FloatTensor(target.size(0), int(max_n_objects), target.size(2), target.size(3)).zero_()
